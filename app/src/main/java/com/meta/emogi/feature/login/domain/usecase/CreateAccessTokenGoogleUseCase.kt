...
class CreateAccessTokenGoogleUseCase @Inject constructor(private val repo: IAuthRepository) : UseCase<String, LoginResponse>() {
    override suspend fun execute(idToken: String): AppResult<LoginResponse> {
        val result = repo.createAccessTokenGoogle(idToken)
